...
est1=myPF(model,truth,meas);
est2=myPFsm2(model,truth,meas);
esterror(:,:,i)=[est1.error;est2.error];
end
...
hold on
plot(esterror(5,:));
legend('xerrorPF','xerrorKFwaveletPFSM')
hold off
figure(2)
plot(esterror(3,:));
hold on
plot(esterror(7,:));
legend('yerrorPF','yerrorPFSM')
hold off
x=truth.X;
sx=truth.station;
z=meas.Z;
